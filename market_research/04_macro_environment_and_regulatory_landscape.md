...
Tenstorrent's RISC-V-based AI accelerators represent the most strategic hardware development for Token Factory. Unlike Nvidia (CUDA) and AMD (ROCm), which use proprietary instruction set architectures, Tenstorrent's RISC-V approach uses an open-source instruction set, aligning with the sovereign AI movement's emphasis on supply chain transparency and independence. Infinia Technologies' partnership with Tenstorrent positions Token Factory to be among the first inference platforms to offer Tenstorrent-powered inference, potentially at 30-50% lower cost than Nvidia-based alternatives. This capability — if and when Tenstorrent hardware reaches production quality — would represent a pricing advantage that no competitor without an equivalent hardware partnership can match.
